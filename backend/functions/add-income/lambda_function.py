...
def lambda_handler(event, context):
    ...
    try:
        ...
        return {'statusCode': 500, 'headers': headers, 'body': json.dumps({'error': 'A Database error has occurred'})}
    except Exception as e:
        print(f'Unexpected error: {str(e)}')
        return {'statusCode': 502, 'headers': headers, 'body': json.dumps({'error': 'Internal Server Error'})}